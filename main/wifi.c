...
void wifi_provisioning_start(void) 
{
    ...
    #elif CONFIG_EXAMPLE_PROV_TRANSPORT_SOFTAP
    wifi_prov_mgr_config_t config = {
        .scheme = wifi_prov_scheme_softap,
        .scheme_event_handler = WIFI_PROV_EVENT_HANDLER_NONE
    };
    #else
    #error "No provisioning transport selected!"
    #endif

    #ifdef CONFIG_EXAMPLE_PROV_TRANSPORT_BLE
        ESP_LOGI(TAG, "Using BLE for provisioning");
    #elif CONFIG_EXAMPLE_PROV_TRANSPORT_SOFTAP
        ESP_LOGI(TAG, "Using SoftAP for provisioning");
    #else
        ESP_LOGE(TAG, "No provisioning transport selected! Set one in menuconfig.");
        return;
    #endif
    ...
    wifi_prov_security_t security = WIFI_PROV_SECURITY_0; // Default to no security
    const char *pop = NULL;
